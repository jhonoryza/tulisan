...
requires fork [llama.cpp-tq3](https://github.com/turbo-tan/llama.cpp-tq3) or [llama.cpp-hip-turboquant-tq3](https://github.com/flamme-demon/llama.cpp-hip-turboquant-tq3):
...
requires fork [llama-cpp-turboquant](https://github.com/TheTom/llama-cpp-turboquant):
...
requires fork [llama ik_llama.cpp](https://github.com/ikawrakow/ik_llama.cpp):
...
## Install ROCM for AMD RX6600 GPU
...
## How to Build llama.cpp (Original, llama-cpp-turboquant Fork, llama.cpp-tq3 Fork)
...
## How to Build llama.cpp Fork ik_llama.cpp
...
Note: ik_llama.cpp does not support ROCM, so the build fails
...
| Flag | Technical explanation | Impact / Why it's used |
|------|-------------------|-------------------------|
...
| `-m /root/models/Qwen_Qwen3.6-35B-A3B-Q4_K_M.gguf` | **Model file** yang akan dipakai. Format **GGUF** (`q4_k_m` significa 4‑bit quantization, level `k`). Model berukuran sekitar 20 GB. | Model di‑load di sini; `--m` or `--model` (setara). |
...
| `-c 256000` | **Context size** – sebesar **256 000** token (karena flag `c` or `--ctx-size`). | Lebih besar dari contoh sebelumnya (131 072). Ini memungkinkan *generation* or *retrieval* dengan **256 k token** (≈ 500 KB * 256k ≈ 488 MB* hanya untuk token buffer; selain itu memakan memori cache). |
...
| Flag | Penjelasan (ulang) | Additional notes |
|------|-------------------|---------------------|
...
| `--mlock` | Lock seluruh memori virtual ke RAM (tidak swap). | Harus di‑pair dengan `--ulimit memlock=-1` or `cap-add=IPC_LOCK` pada Docker; hasilnya memori tidak dipindahkan ke swap. |
...
> - CPU hanya digunakan untuk MoE or I/O (jika ada).
...
## LM Studio Service Example

Create file at `/etc/systemd/system/lmstudio.service`
